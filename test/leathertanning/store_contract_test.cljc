(ns leathertanning.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `leathertanning.store` ns docstring for why a second
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:grade :full-grain}})
    (is (= :full-grain (:grade (store/batch s "batch-001"))))
...
(deftest shipment-propose-commits-and-advances-sequence-and-batch-area
  (let [s (seeded)]
    ...
    (is (= 150.0 (:shipped-area-sqm (store/batch s "batch-001")))
