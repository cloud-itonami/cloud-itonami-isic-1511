(ns leathertanning.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control tanning/finishing-line equipment
  directly... does NOT authorize an effluent discharge') implemented
  faithfully. The single invariant under test:

    LeatherTanningAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Leather Tanning Plant
    Operations Governor would reject; `:schedule-maintenance`/`:flag-
    safety-concern`/`:coordinate-shipment` NEVER auto-commit at any
    phase; `:log-production-batch` (no physical/environmental risk)
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:grade :full-grain}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :full-grain (:grade (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-tanning-drum :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest shipment-area-exceeded-is-held-and-unoverridable
  (testing "a shipment proposal whose area would exceed the batch's own logged area -> HOLD"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])))
      (is (not= :interrupted (:status res)))
      (is (some #{:shipment-area-exceeded} (-> (store/ledger db) last :basis)))
      (is (empty? (store/shipment-history db))))))

(deftest discharge-authorize-is-held-and-permanently-blocked
  (testing "a proposal that sets :discharge-authorize? true -> HOLD, PERMANENT, never reaches request-approval even though the equipment is verified and registered"
    (let [[db actor] (fresh)
      ...
      (is (not= :interrupted (:status res)))
      (is (some #{:discharge-authorize-blocked} (-> (store/ledger db) last :basis)))
      (is (empty? (store/maintenance-history db))))))
  ...
  (testing "scheduling the SAME maintenance record twice -> HOLD on the second attempt"
    (let [[db actor] (fresh)
          _ (exec-op actor "t9a" {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                                  :value {:equipment-id "equip-001" :maintenance-type :tanning-drum-inspection
                                          :scheduled-date "2026-08-01" :discharge-authorize? false}} coordinator)
          _ (approve! actor "t9a")
...
(deftest invalid-shrinkage-temperature-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:shrinkage-temperature-c 250.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-shrinkage-temperature} (-> (store/ledger db) last :basis)))
    (is (not= 250.0 (:shrinkage-temperature-c (store/batch db "batch-001"))) "fabricated shrinkage-temperature never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "equip-001" :severity :moderate
                                            :description "chromium VI odor near tanning drum"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest coordinate-shipment-always-needs-approval
  (testing "a CLEAN shipment coordination is never auto-eligible -- always escalates, even below any area threshold"
    (let [[db actor] (fresh)
