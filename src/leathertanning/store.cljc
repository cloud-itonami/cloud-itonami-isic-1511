(ns leathertanning.store
  "SSoT for the leather-tanning plant-operations coordination actor,
  ...
  this vertical is self-contained (no external leathertanning
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             hide-lot/tanning-process/leather-grade/
                             area/shrinkage-temperature record.
                             ...
                             tannery's production ledger;
                             `:shipped-area-sqm` tracks the batch's
                             own cumulative-shipped ground truth.
    - `equipment`         -- a tanning-drum/finishing-line unit's own
                             ...
                             `batches`.
    - `maintenance`       -- a scheduled tanning-drum/finishing-line
                             maintenance-window DRAFT against a piece
                             of equipment (`leathertanning.registry`'s
    ...
    - `shipments`         -- a proposed outbound leather-shipment
                             DRAFT (`leathertanning.registry`'s
                             `register-shipment`).
  ...
  The ledger stays append-only: 'which hide-lot batch was logged,
  which maintenance was scheduled against a verified/registered
  equipment unit, which shipment was coordinated and at what
  independently-recomputed area, approved by whom, which safety
  concern was flagged' is always a query over an immutable log -- the
  audit trail a tannery owner or leather buyer trusting this
  coordinator needs."
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (leathertanning.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (leathertanning.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :grade :full-grain :hide-source "Cattle Hide - EU Grade A"
                :tanning-process :chrome :area-sqm 500.0 :shrinkage-temperature-c 118.0
                :verified? true :registered? true
                :shipped-area-sqm 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :grade :top-grain :hide-source "Calf Hide - Domestic"
                :tanning-process :vegetable :area-sqm 80.0 :shrinkage-temperature-c 82.0
                :verified? true :registered? true
                :shipped-area-sqm 75.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :grade :split-leather :hide-source "Cattle Hide - Unverified Lot"
                :tanning-process :chrome :area-sqm 600.0 :shrinkage-temperature-c 105.0
                :verified? false :registered? false
                :shipped-area-sqm 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :tanning-drum
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "equip-002" {:id "equip-002" :kind :finishing-equipment
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `leathertanning.registry` and
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `leathertanning.registry` and
  returns {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defrecord MemStore [a]
  ...
  (commit-record! [s {:keys [effect path value] :as record}]
    (cond
      ...
      (let [shipment-id (first path)
        ...
        (swap! a (fn [state]
                   (-> state
                       ...
                       (update-in [:batches batch-id :shipped-area-sqm]
                                  (fn [prev]
                                    (+ (double (or prev 0.0))
                                       (double (or (:area-sqm value) 0.0))))))))
        result)
      ...
      ;; above (also what sibling actors expose as their own low-level
      ;; commit path).
...
(defn sample-data!
  ...
  shipped (a small new shipment blows through its own logged output
  area -- HARD hold), one UNVERIFIED/unregistered batch (blocks any
  shipment coordinated against it); one verified+registered
  tanning-drum unit (schedulable for maintenance), one
  UNVERIFIED/unregistered finishing-equipment unit (blocks any
