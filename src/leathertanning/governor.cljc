(ns leathertanning.governor
  "Leather Tanning Plant Operations Governor -- the independent
  compliance layer that earns the LeatherTanningAdvisor the right to
  ...
  maintenance proposal secretly tries to AUTHORIZE an effluent
  discharge (rather than merely draft-schedule a maintenance window),
  whether a shipment proposal's own claimed area would blow through
  the batch's own logged output area, or when an act stops being a
  coordination proposal and becomes direct tanning/finishing-line-
  equipment control, so this MUST be a separate system able to
  *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is `:leather-tanning-plant-operations-
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated
                                       `:tanning-drum/actuate` or
                                       `:finishing-line/actuate`) is
                                       the 'direct tanning/finishing-
    ...
    7. Batch not verified/
       registered                  -- for `:coordinate-shipment`,
                                       ...
                                       Also part of the 'tannery/batch
                                       record' HARD invariant: a
                                       batch's own verified/registered
                                       status is as much a ground-
                                       truth fact as an equipment
                                       unit's own.
    8. Shipment area exceeded      -- for `:coordinate-shipment`,
                                       INDEPENDENTLY recompute whether
                                       the batch's own recorded
                                       `:shipped-area-sqm` plus the
                                       proposal's own claimed
                                       `:area-sqm` would exceed the
   ...
   10. Invalid shrinkage
       temperature                 -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:shrinkage-temperature-c` that
                                       is not a physically plausible
                                       hydrothermal-stability reading
                                       (`leathertanning.registry/
   ...
   11. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       tannery supervisor. SOFT: the
                                       human may approve."
  (:require [leathertanning.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct tanning/
  finishing-line-equipment-control or effluent-discharge-authorize
  effect."
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct tanning-drum/finishing-line-equipment control,
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") はタンニングドラム・仕上げ設備の直接操作/排水放流の許可確定に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-not-verified-violations
  ...
  ('tannery/batch record must be independently verified/registered
  before any action')."
  [{:keys [op]} proposal st]
...
(defn- batch-not-verified-violations
  ...
  trust the advisor's own report. Also part of the 'tannery/batch
...
(defn- shipment-area-exceeded-violations
  "For `:coordinate-shipment`, INDEPENDENTLY recompute whether the
  batch's own recorded shipped-to-date area plus the proposal's own
  claimed area would exceed the batch's own recorded `:area-sqm` --
  ground truth from the batch's own permanent fields, never a
  self-reported area claim."
  [{:keys [op]} proposal st]
  (when (= op :coordinate-shipment)
    (let [{:keys [batch-id area-sqm]} (:value proposal)
          b (and batch-id (store/batch st batch-id))]
      (cond
        ...
        (not (registry/shipment-area-exceeded-checkable? b area-sqm))
        [{:rule :shipment-area-exceeded
          :detail "生産量/既存出荷実績/申請量のいずれかが数値として確定できない -- 空き容量を検算できないため出荷しない"}]

        (registry/shipment-area-exceeded? b area-sqm)
        [{:rule :shipment-area-exceeded
          :detail (str batch-id " の記録済み生産面積(" (:area-sqm b)
                       "㎡)を、既存出荷実績(" (:shipped-area-sqm b 0.0)
                       "㎡)+今回申請(" area-sqm "㎡)が超過")}]))))
...
(defn- invalid-shrinkage-temperature-violations
  "For `:log-production-batch`, if the patch declares a
  `:shrinkage-temperature-c` that is not a physically plausible
  hydrothermal-stability reading, reject rather than let
  fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [ts (:shrinkage-temperature-c (:value proposal))]
      (when (and (some? ts) (not (registry/shrinkage-temperature-valid? ts)))
        [{:rule :invalid-shrinkage-temperature
          :detail (str ts "°C は物理的に妥当な収縮温度(Ts)の範囲外")}]))))

(defn check
  "Censors a LeatherTanningAdvisor proposal against the governor
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (shipment-area-exceeded-violations request proposal st)
                           (invalid-grade-violations request proposal)
                           (invalid-shrinkage-temperature-violations request proposal)))
