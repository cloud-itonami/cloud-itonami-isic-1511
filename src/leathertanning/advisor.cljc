(ns leathertanning.advisor
  "LeatherTanningAdvisor -- the *contained intelligence node* for the
  leather-tanning plant-operations coordination actor.

  It normalizes production-batch patches (hide-lot/tanning-process/
  leather-grade/area/shrinkage-temperature), drafts a tanning-drum/
  finishing-line maintenance proposal against a piece of equipment,
  drafts a safety-concern flag, and drafts an outbound leather-
  ...
  a real tanning/finishing-line-equipment actuation, effluent-
  discharge authorization, or freight dispatch. Every output is
  censored downstream by `leathertanning.governor` before anything
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `leathertanning.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct tanning/
                                 ; finishing-line-equipment-control or
                                 ; effluent-discharge-authorize effect
  ...
  `leathertanning.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  ...
  validates the patch; it does not invent the batch's hide-lot,
  tanning-process, grade, area or shrinkage-temperature or
...
(defn- schedule-maintenance
  "Draft a tanning-drum/finishing-line maintenance-window scheduling
  ...
  `leathertanning.governor` NEVER trusts this report -- it
    ...
    {:summary    (str subject " 向け保守作業予定提案 (" (:maintenance-type value) ")"
     ...
     :rationale  (if eq
                   (str "equipment-verified?=" (registry/equipment-verified? eq)
                        ...
                        " discharge-authorize?=" (boolean (:discharge-authorize? value)))
                   (str equipment-id " が見つかりません"))
     :cites      (if eq [equipment-id] [])
     :effect     :maintenance/schedule
     :value      value
     :stake      nil
     :confidence (if (and ready? (not (:discharge-authorize? value))) 0.9 0.3)}))

(defn- flag-safety-concern
  "Draft a chemical-hazard (chromium VI)/effluent-discharge/worker-
  safety concern flag. ALWAYS `:stake :coordination/safety-concern` --
  ...
  administrative technicality). See `leathertanning.phase`: no phase
  ever adds this op to a phase's `:auto` set; `leathertanning.governor`
...
(defn- coordinate-shipment
  "Draft an outbound leather-shipment coordination proposal against a
  production batch. The advisor passes through the caller's own
  claimed area -- it does NOT invent one, and `leathertanning.governor`
  NEVER trusts it: it independently recomputes whether the batch's own
  cumulative-shipped area plus this claim would exceed the batch's own
  recorded area before any commit is possible."
  [db {:keys [subject value]}]
  (let [batch-id (:batch-id value)
        b (store/batch db batch-id)
        ready? (and b (registry/batch-ready? b))
        over-area? (and b (registry/shipment-area-exceeded?
                           b (:area-sqm value)))]
    {:summary    (str subject " 向け出荷調整提案 ("
     ...
     :value      value
     :stake      nil
     :confidence (if (and ready? (not over-area?)) 0.9 0.3)}))
...
(def ^:private system-prompt
  (str "あなたは製革工場プラント運用コーディネーターの助言者です。"
       ...
       "排水放流の許可(discharge-authorize)確定やタンニングドラム・仕上げ設備等の設備の直接操作を絶対に"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `leathertanning.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :leathertanning-advisor-proposal
   :op         (:op request)
