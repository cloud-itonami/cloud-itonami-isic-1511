(ns leathertanning.operation
  "LeatherTanningOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (LeatherTanningAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Leather Tanning Plant Operations Governor (:govern) and the rollout
  ...
  each operation is auditable and checkpointed. A tannery's lifecycle
  ...
  decision to a human tannery supervisor/shipping approver. The approver
  resumes with `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [leathertanning.advisor :as advisor]
...
(defn build
  "Compiles a LeatherTanningOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `leathertanning.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; LeatherTanningAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Leather Tanning Plant Operations Governor -- independent censor.
      (g/add-node :govern
