(ns leathertanning.phase
  "Phase 0->3 staged rollout for the leather-tanning plant-operations
  coordination actor.
    ...
    Phase 3  supervised-auto    -- adds maintenance scheduling (still
                                    ...
                                    physical/environmental risk) may
                                    auto-commit.
  ...
  consequence (a tanning-drum/finishing-line maintenance window means
  production downtime, and the equipment ends up actually touched); it
  is always a human tannery supervisor's call.
  `leathertanning.governor`'s `discharge-authorize-blocked-violations`
  HARD-blocks authorize attempts unconditionally, and the confidence/
...
(defn verdict->disposition
  "Map a Leather Tanning Plant Operations Governor verdict to a base
