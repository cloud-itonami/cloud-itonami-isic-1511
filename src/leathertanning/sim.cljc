(ns leathertanning.sim
  "Demo driver -- `clojure -M:dev:run`. Walks a clean tannery through
  ...
  shipment proposal that would exceed the batch's own logged output
  area, a proposal that tries to AUTHORIZE an effluent discharge
  ...
  grade, and a production-batch patch with an implausible
  shrinkage-temperature reading.
  ...
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println (exec-op actor "t1"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:grade :full-grain :last-assessed "2026-07-15"}}
                       coordinator))

    (println "== schedule-maintenance mnt-1 on equip-001 (verified, registered, tanning-drum -- escalates, approve) ==")
    (let [r (exec-op actor "t2"
                      ...
                      coordinator)]
      (println r)
      (println "-- human tannery supervisor approves --")
    ...
    (println (exec-op actor "t6"
                       {:op :actuate-tanning-drum :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on equip-002 (UNVERIFIED/unregistered finishing equipment -> HARD hold) ==")
    ...
    (println "== coordinate-shipment ship-3 on batch-002 (10 sqm would exceed area 80 vs shipped 75 -> HARD hold) ==")
    ...
    (println "== log-production-batch batch-001 with an implausible shrinkage-temperature reading -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:shrinkage-temperature-c 250.0}}
                       coordinator))
